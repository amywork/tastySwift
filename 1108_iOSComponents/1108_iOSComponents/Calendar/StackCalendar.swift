...
struct CalendarDataModel {
    ...
    init?(date: Date) {
        ...
        self.year = components.year ?? 2017
        ...
        let val = weekDayCompo.weekday ?? 1
        startWeekDayOfMonth = WeekDay.init(rawValue: val - 1)!
...
class CalendarManager {
    ...
    class func nextMonth(with currentDate:Date) -> Date? {
        ...
        guard let newDate = calendar.date(byAdding: addCompo, to: currentDate) else { return nil }
        return newDate
    }
...
extension CalendarView: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if indexPath.section == 0 {
            ...
            if changedIndex+1 == calendarDateModel!.day {
                cell.highlight()
            }
            if changedIndex+1 == calendarDateModel!.day + 12 {
                cell.higlightRed()
            }
        }
